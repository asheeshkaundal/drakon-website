...
export default function ProductDetailClient({ ball }: { ball: Ball }) {
  ...
  const discounted = Math.round((ball.original_Price ?? 0) * (1 - ((ball.discount_Percentage || 0) / 100)));
  const formatPrice = (v: number) =>
    new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR", maximumFractionDigits: 0 }).format(Math.round(v));

  useEffect(() => {
    // keyboard close for zoom
  ...
  function handleAddToCart() {
    addToCart({ id: ball.id, name: ball.name, price: discounted, image: ball.image });
    setAdded(true);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <nav className="mb-6 text-sm text-gray-600">
        <Link href="/premium-balls" className="hover:underline">Premium Balls</Link>
        <span className="mx-2">/</span>
        <span aria-current="page" className="font-semibold">{ball.name}</span>
      </nav>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div className="space-y-4">
          ...
          >
            <Image src={selectedImage} alt={ball.name} width={1200} height={900} className="object-cover w-full h-[420px] sm:h-[520px] transition-transform duration-300" />
            <div className="absolute top-4 right-4 bg-cricket-red text-white px-3 py-1 rounded-full text-sm font-bold">NEW</div>
          </div>

          {/* Thumbnails */}
          <div ref={thumbnailsRef} className="flex gap-3 overflow-x-auto py-1">
            {[ball.image, /* extra placeholders: could be more images */].map((src, idx) => (
              <button
                key={idx}
                onClick={() => setSelectedImage(src)}
                className={`flex-shrink-0 w-20 h-20 rounded-md overflow-hidden border ${selectedImage === src ? "ring-2 ring-cricket-red" : "border-gray-200"} focus:outline-none`}
        ...
        <div className="sticky top-6">
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="flex items-center border rounded-lg p-1 bg-white">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (!Number.isNaN(val)) setQuantity(Math.max(1, Math.floor(val)));
                }}
            ...
            <Button
              ...
              className={`flex items-center gap-2 px-4 py-2 ${added ? "bg-green-600" : "bg-navy-blue"} text-white`}
            >
              {added ? <><Check className="w-4 h-4" /> Added</> : <><ShoppingCart className="w-4 h-4" /> Add to cart</>}
            </Button>
          </div>

          {/* small note & share/placeholders */}
